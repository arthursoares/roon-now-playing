...
│   │       ├── clientNames.ts    # Name persistence
...
| `cover` | **Ultra-minimal** - Centered artwork only, white/black background |

## Key Features

### Self-Service Onboarding
- **Auto-generated friendly names** for screens (adjective-animal-number format, e.g., `gentle-fox-17`)
- **QR code welcome screen** replaces Roon-specific "Waiting for Roon Core..." message
- **Per-screen config page** at `/admin/screen/:friendlyName` — mobile-friendly, focused UI
- Name generator in `packages/server/src/nameGenerator.ts`, names persisted in `client-names.json`
- QR codes generated client-side using `qrcode` npm package (zero server config needed)
...
- **2026-02**: Self-service welcome screen with QR code onboarding and per-screen config page
