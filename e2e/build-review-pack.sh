...
for res_dir in "$MATRIX_DIR"/*/; do
  ...
  # collect distinct layout prefixes (filename is <layout>__<background>.png)
  layouts="$(find "$res_dir" -maxdepth 1 -name '*__*.png' -exec basename {} \; \
            | sed 's/__.*//' | sort -u)"
  [ -z "$layouts" ] && continue
  ...
  for layout in $layouts; do
    args=()
    for f in "$res_dir$layout"__*.png; do
      [ -e "$f" ] || continue
      bg="$(basename "$f" .png | sed "s/^${layout}__//")"
    ...
    montage "${args[@]}" \
      ...
      -fill '#e4e4e7' \
      -pointsize 16 \
      "$OUT_ROOT/$res/$layout.png"
    sheet_count=$((sheet_count + 1))
    echo "  built $res/$layout.png"
  done
done
